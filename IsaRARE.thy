...
\item Reconstruction:
 If a rule is used in a proof certificate by an external solver, the generated lemmas can be used by
...
text \<open>
This file enables the RARE extension for using variables annotated as :list.
\<close>
...
This information can make a lemma quite convoluted. The process can be turned off with the option
